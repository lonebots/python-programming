...
class Solution:
    def maxSumOfNodes(self, index, isEven, nums, k, memo):
        if index == len(nums) :
            # if action is performed on odd number of elements return INT_MIN
        ...
        # XOR done
        XOR = (nums[index] ^ k) + self.maxSumOfNodes(index + 1, isEven ^ 1, nums, k, memo)

        # memoize and return result 
        memo[index][isEven] = max(noXOR, XOR) 
        return memo[index][isEven]
